...
import { Deployment, DomainMapping, LogEntry, Project, ProjectStatus } from './types';
...
export default function App() {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setSuccess('Cerebras key saved on backend.');
      setCerebrasKey('');
    } catch (err: any) {
  ...
  const stats = useMemo(() => {
    ...
    const failed = projects.filter((project) => project.deployments?.[0]?.status === 'failed').length;
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 bg-card-bg border border-border rounded-xl p-5 space-y-4">
                <h2 className="font-bold text-lg">Active deployments</h2>
                  ...
                  {projects.map((project) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className="text-left border border-border rounded-xl p-4 bg-bg"
                      >
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {domainMappings.slice(0, 8).map((mapping) => (
                    <div key={mapping.id} className="border border-border rounded-lg p-3 bg-bg">
                      <p className="font-semibold">{mapping.domain}</p>
        ...
        {tab === 'deploy' && !selectedProject && (
          <section className="max-w-4xl rounded-xl bg-card-bg border border-border p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Input label="FreeDomain domain" value={form.freeDomainDomain} onChange={(value) => setForm((prev) => ({ ...prev, freeDomainDomain: value }))} placeholder="yourdomain.example" />
              <Input label="FreeDomain DNS API key" type="password" value={form.freeDomainDnsApiKey} onChange={(value) => setForm((prev) => ({ ...prev, freeDomainDnsApiKey: value }))} />
            </div>
            <button onClick={submitDeploy} disabled={deploying} className="bg-primary text-white px-5 py-2 rounded-lg text-sm font-semibold inline-flex items-center gap-2 disabled:opacity-50">
    ...
    <section className="space-y-4">
      ...
      <div className="rounded-xl bg-card-bg border border-border p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 text-sm">
          ...
          <InfoCard label="Current live URL" value={current?.public_url ? <a className="inline-flex items-center gap-1 text-primary" href={current.public_url} target="_blank" rel="noreferrer">Open <ExternalLink className="w-3 h-3" /></a> : 'Pending'} />
          <InfoCard label="Detected framework/runtime" value={current?.detected_framework ?? project?.detected_framework ?? 'Detecting'} />
          <InfoCard label="Detected branch" value={current?.detected_branch ?? project?.detected_branch ?? 'Detecting'} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <div className="mt-3 space-y-2 text-sm">
            ...
            {deployments.map((deployment) => (
              <div key={deployment.id} className="border border-border rounded-lg p-3 bg-bg">
                ...
                <p className="text-xs text-text-muted mt-1">{new Date(deployment.created_at).toLocaleString()}</p>
        ...
        <div className="rounded-xl bg-sidebar-bg p-5 h-[28rem] overflow-y-auto">
          <h3 className="text-xs text-white uppercase font-bold tracking-wider">Logs / history</h3>
